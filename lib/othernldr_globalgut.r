...
##### Load Data #####
# Cleaned datasets
dat_hg <- read.delim(paste0(current_dir, "data/global_gut/421_clean_otus.txt"),
                     header=T, sep="\t", quote="\"", row.names=1, comment.char="", as.is=T)
meta_hg <- read.delim(paste0(current_dir, "data/global_gut/clean_map.txt"),
...
d_hg <- read.delim(paste0(current_dir, "data/global_gut/clean_uw_unifrac_dist.txt"),
...
# Populations - exported as 650x600
ggplot(pc_df, aes(x=-Dim1, y=-Dim2, color=Population)) +
    ...
    theme_classic() +
    theme(plot.title = element_text(size=16, face="bold")) + NULL
# Age
...
lgd_hg <- lg.dist(d_hg, neighborhood.size = 20, weighted=TRUE)
...
# tSNE with LGD - note this currently overwrites tsne results above.
tsne <- Rtsne(lgd_hg)
tsne_df <- as.data.frame(tsne$Y)
rownames(tsne_df) <- rownames(as.matrix(d_hg))
colnames(tsne_df) <- paste0("Dim", 1:ncol(tsne_df))
...
ggplot(tsne_df, aes(x=Dim1, y=Dim2, color=Population)) +
  geom_point(size = 4, alpha=0.8) +
  scale_color_manual(values = pop_cols) +
  labs(title = "LGD & t-SNE: Human Microbiome Project", x = "Dimension 1", y = "Dimension 2") +
  theme_classic() + 
  theme(plot.title = element_text(size=16, face="bold")) + NULL
ggplot(tsne_df, aes(x=Dim1, y=Dim2, color=Age)) +
  geom_point(size = 4, alpha=0.8) +
